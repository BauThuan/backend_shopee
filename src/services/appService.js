  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!inforBook) {
        ...
        if (inforBook.image) {
          inforBook.image = new Buffer(inforBook.image, "base64").toString(
            "binary"
          );
        }
...
let getLengthListBook = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      let { categoryId } = data;
      let listBooks = await db.Book.findAll({
        where: { categoryId: categoryId },
      });
      if (listBooks) {
        resolve({
    ...
    try {
      ...
      let categories = await db.Allcode.findAll({
        where: {
          type: "CATEGORY",
          [Op.or]: [
            { valueEn: { [Op.like]: `%${searchContent}%` } },
            { valueVi: { [Op.like]: `%${searchContent}%` } },
          ],
        },
