          ...
          if (checkBook && checkInfor) {
            ...
            if (checkBook && !checkInfor) {
              resolve({
                errCode: 1,
                messageEN:
                  "Book created successfully but book information creation failed",
                messageVI:
                  "Đã tạo sách thành công nhưng tạo thông tin sách thất bại",
              });
            }
            if (!checkBook && checkInfor) {
              resolve({
                errCode: 1,
                messageEN:
                  "Book information created successfully but book creation failed",
                messageVI:
                  "Đã tạo thông tin sách thành công nhưng tạo sách thất bại",
              });
            }
            resolve({
              errCode: 1,
...
let updateBookById = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let bookInfor = await db.Book_Infor.findOne({
      ...
      if (
        ...
        if (checkBook && checkInfor) {
          ...
          resolve({
            errCode: 1,
            messageEN:
              "Book update successful but book extra information update failed",
            messageVI:
              "Cập nhật sách thành công nhưng thông tin chi tiết sách cập nhật thất bại",
          });
        } else if (!checkBook && checkInfor) {
          resolve({
            errCode: 1,
            messageEN:
              "Update book details successfully but book information has not been saved",
            messageVI:
              "Cập nhật thông tin chi tiết về sách thành công nhưng thông tin về sách chưa được lưu",
          });
        } else {
          resolve({
            errCode: 1,
        ...
        let checkRoomIdExist = await db.Book_Infor.findOne({
          where: { roomId: roomId },
        });
        if (checkRoomIdExist) {
          resolve({
        ...
        if (check) {
          resolve({
            errCode: 1,
            messageEN:
              "There can't be two books with the same title in one subject",
...
let addNewAuthor = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let check = await db.Author.create({
        name,
        image,
        birthDay,
        description,
        ...
        if (carts.length > 0) {
          carts = carts.map((item) => {
            if (item.userCartData && item.userCartData.image) {
              item.userCartData.image = new Buffer(
      ...
      if (!checkCart && !bookSelected) {
        ...
        if (!check && !checkUpdateBook) {
          ...
          resolve({
            errCode: 1,
            messageEN:
              "Confirm that the request to borrow a book is successful but the book information update failed",
            messageVI:
              "Xác nhận yêu cầu mượn sách thành công nhưng cập nhật thông tin sách thất bại",
          });
        } else if (check && !checkUpdateBook) {
          resolve({
            errCode: 1,
            messageEN:
              "Confirm that the request to borrow a book failed but update the book information successfully",
            messageVI:
              "Xác nhận yêu cầu mượn sách thất bại nhưng cập nhật thông tin sách thành công",
          });
        } else {
          resolve({
            errCode: 0,
    ...
    try {
      ...
      if (!listHistory) {
        ...
        resolve({
          errCode: 0,
          messageEN:
            "Get a list of successful transactions during this time period",
          messageVI:
            "Lấy danh sách lịch sự giao dịch trong khoảng thời gian này thành công",
          data: listHistory,
        });
      }
    } catch (error) {
...
let addNewClother = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      let { name, status, purchases, price, percent, image } = data;
      const checkAddClother = await db.Book.create({
        name: name,
        status: status,
        purchases: purchases,
        price: price,
      ...
      if (!checkAddClother) {
        resolve({
          success: false,
          message: "creat new failure !",
        });
      } else {
        resolve({
          success: true,
          message: "create new succesfully !",
        });
      }
    } catch (error) {
      reject(error);
