  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (emailCheck) {
        ...
        if (check) {
          //   await emailService.sendEmailRegister({
          //     reciverEmail: email,
          //     language,
          //     link,
          //   });
          resolve({
            errCode: 0,
            messageEN:
              "Create new Accoount successfully, Please check your email to continue with the registration procedure ",
            messageVI: "Thêm tài khoản mới thành công",
          });
        } else {
          resolve({
            errCode: 2,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (emailCheck) {
        ...
        if (emailCheck.image) {
          emailCheck.image = new Buffer(emailCheck.image, "base64").toString(
            "binary"
          );
        }
    ...
    try {
      ...
      if (emailCheck) {
        await emailService.sendEmailForgotPassword({
          reciverEmail: email,
          phoneNumber: emailCheck.phoneNumber,
          language: language,
        });
        resolve({
          errCode: 0,
          messageEN:
            "Please check your email to continue with the forgotten password procedure",
          messageVI:
            "Vui lòng kiểm tra email để tiếp tục thủ tục quên mật khẩu",
        });
      } else {
        resolve({
          errCode: 1,
      ...
      if (carts) {
        carts = carts.map((item) => {
          if (item.bookCartData.image) {
            item.bookCartData.image = new Buffer(
              item.bookCartData.image,
              "base64"
            ).toString("binary");
          }
          return item;
...
let addOneNotifycation = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      let { senderId, titleId, messageEn, messageVi, location, receiverId } =
        data;
...
let addOneCart = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let checkCartDoing = await db.Cart.findOne({
        where: { userId: userId, bookId: bookId, statusId: "W" },
      });
      ...
      if (!checkBook) {
        ...
        resolve({
          errCode: 1,
          messageEN:
            "This book you have requested to borrow and is in the process of being processed, so you cannot add this book to your cart",
          messageVI:
            "Quyển sách này bạn đã yêu cầu mượn và đang trong quá trình sử lý nên bạn không thể thêm sách này vào giỏ",
        });
      } else if (!checkCartDoing) {
        let checkBookBorrowed = await db.Cart.findOne({
          where: { userId: userId, bookId: bookId, statusId: "B" },
        });
        if (checkBookBorrowed) {
          resolve({
            errCode: 1,
            messageEN:
              "This book you have requested to borrow and is in the process of being borrowed, so you cannot add it to your cart",
            messageVI:
              "Quyển sách này bạn đã yêu cầu mượn và đang trong quá trình mượn nên bạn không thể thêm vào giỏ ",
          });
        } else {
          ...
          if (checkExist) {
            ...
            let checkNumber = await db.Cart.findAll({
              where: { userId: userId, statusId: "IN" },
            });
...
let borrowNowCart = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let checkNumber = await db.Cart.findAll({
        where: { userId: userId, statusId: "B" },
      });
      let checkBook = await db.Book.findOne({ where: { id: bookId } });
      if (!checkBook) {
        resolve({
        ...
        if (!checkExist) {
          ...
          if (checkCreate) {
            resolve({
              errCode: 0,
              messageEN:
                "Your request has been fulfilled, please wait a few minutes for the staff to review your request to borrow books",
              messageVI:
                "Yêu cầu của bạn đã được thực hiện vui lòng đợi vài phút để nhân viên xem sét yêu cầu mượn sách của bạn",
            });
          } else {
            resolve({
              errCode: 1,
              messageEN:
                "Your request has failed during submission, please check again or notify admin about your problem.",
              messageVI:
                "Yêu cầu của bạn đã bị lỗi trong quá trình gửi đi vui lòng kiểm tra lại hoặc thông báo cho admin biết về vấn đề của bạn",
            });
          }
          ...
          resolve({
            errCode: 1,
            messageEN:
              "This book is in the process of borrowing, so you can't borrow it anymore",
            messageVI:
              "Quyển sách này bạn đang trong quá trình mượn nên bạn không thể mượn tiếp ",
          });
        } else if (checkExist && checkExist.statusId === "W") {
          resolve({
            errCode: 1,
            messageEN:
              "Your request to borrow the book is already in the process of being processed, so you cannot borrow this book anymore",
            messageVI:
              "Yêu cầu mượn sách của bạn đã trong quá trình sử lý nên bạn không thể mượn thêm sách này",
          });
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let checkExist = await db.Notifycation.findOne({
        where: { senderId: senderId, message: { [Op.like]: `%${message}%` } },
      });
      if (checkExist) {
        let check = await checkExist.destroy();
        if (check) {
        ...
        include: [
          {
            model: db.Book,
            as: "bookCartData",
            attributes: ["name", "image", "borrowed", "quantity"],
            include: [
              { model: db.Author, as: "authorData", attributes: ["name"] },
            ],
          },
