...
let postOneMessageByRoom = async (req, res) => {
  try {
    ...
    let response = await appService.postOneMessageByRoom({
      ...req.body,
      userId: req.userId,
    });
    return res.status(200).json(response);
...
let deleteOneMessageByRoom = async (req, res) => {
  try {
    ...
    let response = await appService.deleteOneMessageByRoom({
      ...req.query,
      userId: req.userId,
    });
    return res.status(200).json(response);
...
let updateOneMessageByRoom = async (req, res) => {
  try {
    ...
    let response = await appService.updateOneMessageByRoom({
      ...req.body,
      userId: req.userId,
    });
    return res.status(200).json(response);
    ...
    if (comments) {
      comments = comments.map((item) => {
        if (item.userCommentData.image) {
          item.userCommentData.image = new Buffer(
            item.userCommentData.image,
            "base64"
          ).toString("binary");
        }
        return item;
